...
function getHorizontalRuleDomStyle(lineStyle, lineColor) {
    switch (lineStyle) {
        case 'dotted':
        ...
        paragraph: {
            ...
            toDOM(node) {
                ...
                if (node.attrs.firstLineIndent)
                    style.push(`text-indent:${node.attrs.firstLineIndent}em`);
                if (node.attrs.indent)
                    style.push(`margin-left:${node.attrs.indent * 2}em`);
                if (node.attrs.rightIndent)
                    style.push(`margin-right:${node.attrs.rightIndent * 2}em`);
                if (node.attrs.lineHeight !== 1.5)
                    style.push(`line-height:${node.attrs.lineHeight}`);
                if (node.attrs.spaceBefore)
                    style.push(`margin-top:${node.attrs.spaceBefore}pt`);
                if (node.attrs.spaceAfter)
                    style.push(`margin-bottom:${node.attrs.spaceAfter}pt`);
                if (node.attrs.listType)
                    style.push(`--list-level:${node.attrs.listLevel ?? 0}`);
                if (node.attrs.fontSizeHint)
                    style.push(`font-size:${node.attrs.fontSizeHint}pt`);
                if (node.attrs.fontFamilyHint)
                    style.push(`font-family:${node.attrs.fontFamilyHint}`);
                if (node.attrs.headingLevel) {
                    style.push('font-weight:700');
                    if (!node.attrs.fontFamilyHint)
                        style.push(`font-family:${FONT_STACKS.hei}`);
                }
                const cls = [];
                if (node.attrs.listType === 'bullet')
                    cls.push('list-bullet');
                if (node.attrs.listType === 'ordered')
                    cls.push('list-ordered');
                if (node.attrs.listType === 'task')
                    cls.push('list-task');
                if (node.attrs.listType === 'task' && node.attrs.listChecked)
                    cls.push('list-task-checked');
                if (node.attrs.pageBreakBefore)
                    cls.push('page-break-before');
                const domAttrs = {
                    style: style.join(';') || undefined,
                    class: cls.join(' ') || undefined,
                };
                if (node.attrs.listType)
                    domAttrs['data-list-type'] = node.attrs.listType;
                if (node.attrs.listType)
                    domAttrs['data-list-level'] = String(node.attrs.listLevel ?? 0);
                if (node.attrs.listType === 'task')
                    domAttrs['data-list-checked'] = String(Boolean(node.attrs.listChecked));
                if (node.attrs.headingLevel)
                    domAttrs['data-heading-level'] = String(node.attrs.headingLevel);
                if (node.attrs.fontSizeHint)
                    domAttrs['data-font-size-hint'] = String(node.attrs.fontSizeHint);
                if (node.attrs.fontFamilyHint)
                    domAttrs['data-font-family-hint'] = String(node.attrs.fontFamilyHint);
                if (node.attrs.pageBreakBefore)
                    domAttrs['data-page-break'] = 'true';
                return ['p', domAttrs, 0];
            },
        },
        table_of_contents: {
        ...
        textStyle: {
            ...
            toDOM(mark) {
                ...
                if (mark.attrs.fontSize)
                    style.push(`font-size:${mark.attrs.fontSize}pt`);
                if (mark.attrs.color)
                    style.push(`color:${mark.attrs.color}`);
                if (mark.attrs.backgroundColor)
                    style.push(`background-color:${mark.attrs.backgroundColor}`);
                if (mark.attrs.bold)
                ...
                if (mark.attrs.strikethrough)
                    style.push('text-decoration:line-through');
                if (mark.attrs.superscript)
                    style.push('vertical-align:super;font-size:smaller');
                if (mark.attrs.subscript)
                    style.push('vertical-align:sub;font-size:smaller');
                if (mark.attrs.letterSpacing)
                    style.push(`letter-spacing:${mark.attrs.letterSpacing}pt`);
