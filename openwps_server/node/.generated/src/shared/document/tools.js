...
function getParagraphs(doc) {
    ...
    doc.forEach((node, pos) => {
        if (node.type.name === 'paragraph')
            paragraphs.push({ node, pos, index: index++ });
    });
    return paragraphs;
}
...
export function resolveContainsText(doc, range) {
    ...
    for (const paragraph of getParagraphs(doc)) {
        if (Number.isInteger(range?.paragraphIndex) && paragraph.index !== Number(range.paragraphIndex))
            continue;
        if (Number.isInteger(range?.from) && paragraph.index < Number(range.from))
            continue;
        if (Number.isInteger(range?.to) && paragraph.index > Number(range.to))
            continue;
        const scopedIndexes = normalizeIndexes(range?.paragraphIndexes);
        if (scopedIndexes.length > 0 && !scopedIndexes.includes(paragraph.index))
            continue;
...
export function resolveTextRanges(doc, range) {
    if (!Array.isArray(range?.textRanges))
        return [];
    const matches = [];
    range.textRanges.forEach((item, index) => {
        ...
        if (!Number.isInteger(paragraphIndex) || !Number.isInteger(startOffset) || !Number.isInteger(endOffset) || endOffset <= startOffset)
            return;
        ...
        if (typeof item.text === 'string') {
            ...
            const caseSensitive = range?.caseSensitive === true;
            if ((caseSensitive && actual !== expected) || (!caseSensitive && actual.toLocaleLowerCase() !== expected.toLocaleLowerCase()))
                return;
        }
        matches.push({
            paragraph,
...
export function resolveParagraphRange(doc, range, selectionContext) {
    ...
    if (!range?.type || range.type === 'all')
        return paragraphs;
    if (range.type === 'paragraph')
        return paragraphs.filter(p => p.index === Number(range.paragraphIndex));
    if (range.type === 'paragraphs')
        return paragraphs.filter(p => p.index >= Number(range.from ?? 0) && p.index <= Number(range.to ?? Number.POSITIVE_INFINITY));
    ...
    if (range.type === 'first_paragraph')
        return paragraphs.slice(0, 1);
    if (range.type === 'last_paragraph')
        return paragraphs.slice(-1);
    if (range.type === 'odd_paragraphs')
        return paragraphs.filter(p => p.index % 2 === 0);
    if (range.type === 'even_paragraphs')
        return paragraphs.filter(p => p.index % 2 === 1);
    if (range.type === 'contains_text')
        return [...new Map(resolveContainsText(doc, range).map(m => [m.paragraph.index, m.paragraph])).values()];
    if (range.type === 'text_ranges')
        return [...new Map(resolveTextRanges(doc, range).map(m => [m.paragraph.index, m.paragraph])).values()];
...
export function resolveTextMatches(doc, range, selectionContext) {
    if (range?.type === 'contains_text')
        return resolveContainsText(doc, range);
    if (range?.type === 'text_ranges')
        return resolveTextRanges(doc, range);
    if (range?.type === 'selection')
        return resolveSelection(doc, range, selectionContext);
...
export function validateRange(name, range) {
    if (!range || typeof range !== 'object' || Array.isArray(range))
        return `${name} 缺少 range 参数`;
    if (range.type === 'contains_text' && !String(range.text ?? '').trim())
        return `${name} 的 contains_text range 缺少 text`;
    if (range.type === 'text_ranges' && !Array.isArray(range.textRanges))
        return `${name} 的 text_ranges range 缺少 textRanges`;
    return null;
}
...
function representativeTextStyle(node) {
    let attrs = {};
    node.forEach(child => {
        if (Object.keys(attrs).length > 0 || !child.isText)
            return;
...
export function documentContentSnapshot(doc, from, to) {
    ...
    const selectedBlocks = blocks.filter(block => (block.paragraphIndex == null || (block.paragraphIndex >= from && block.paragraphIndex <= to)));
...
function hasMergedCells(tableNode) {
    return tableNode.content.content.some(row => row.content.content.some(cell => (Number(cell.attrs.colspan ?? 1) !== 1 || Number(cell.attrs.rowspan ?? 1) !== 1)));
}
...
function resolveTableTarget(doc, params, selectionContext) {
    ...
    if (!table)
        return { error: `未找到第 ${tableIndex + 1} 个表格` };
...
function executeTableStructureTool(state, tr, toolName, params, selectionContext) {
    ...
    if (hasMergedCells(table.node))
        return fail('当前后端版本暂不支持对含合并单元格的表格增删行列');
    const dims = tableDimensions(table.node);
    const rows = table.node.content.content;
    if (toolName.includes('row')) {
        if (!Number.isInteger(rowIndex) || rowIndex < 0 || rowIndex >= dims.rows)
            return fail('表格行工具需要有效 rowIndex，或当前 selection 位于表格单元格内');
        if (toolName === 'delete_table_row' && dims.rows <= 1)
            return fail('不能删除表格中的最后一行');
        let nextRows;
    ...
    if (!Number.isInteger(columnIndex) || columnIndex < 0 || columnIndex >= dims.cols)
        return fail('表格列工具需要有效 columnIndex，或当前 selection 位于表格单元格内');
    if (toolName === 'delete_table_column' && dims.cols <= 1)
        return fail('不能删除表格中的最后一列');
...
async function measurePageBlockIndexes(doc, pageConfig, pageNumber) {
    ...
    try {
        ...
        if (renderError)
            throw new Error(`前端 headless 渲染失败：${renderError}`);
...
async function getPageBlocks(doc, pageConfig, pageNumber) {
    ...
    try {
        ...
        if (pageNumber > measured.pageCount)
            return { error: `未找到第 ${pageNumber} 页，当前服务端渲染约 ${measured.pageCount} 页` };
        ...
        if (pageNumber !== 1)
            return { error: `未找到第 ${pageNumber} 页，服务端当前只能回退读取第 1 页结构` };
...
async function startStaticDistRendererServer() {
    ...
    if (!address || typeof address === 'string')
        throw new Error('无法启动前端 headless 静态渲染服务');
    return {
        server,
...
async function openFrontendRendererPage(browser, docJson, pageConfig) {
    ...
    let firstPage = null;
    try {
        firstPage = await createFrontendRendererPage(browser, docJson, pageConfig, appendHeadlessRenderParam(process.env.OPENWPS_HEADLESS_RENDERER_URL || 'http://localhost:5174/'));
...
async function capturePageScreenshot(doc, pageConfig, params) {
    ...
    try {
        ...
        const renderError = await renderPage.evaluate(() => window.__OPENWPS_HEADLESS_ERROR__ ?? null);
        if (renderError)
            return fail(`前端 headless 渲染失败：${renderError}`);
        await renderPage.evaluate(() => document.fonts?.ready);
        const metrics = await renderPage.evaluate(() => window.__OPENWPS_HEADLESS_READY__);
        const pageCount = Math.max(1, Number(metrics?.pageCount ?? 1) || 1);
        if (pageNumber > pageCount)
            return fail(`未找到第 ${pageNumber} 页，当前服务端渲染约 ${pageCount} 页`);
...
function locateDocumentImage(doc, pageConfig, params) {
    ...
    if (!target)
        return fail('未找到指定的文档图片', { availableImages: images.map(({ dataUrl, ...image }) => image) });
...
export async function executeDocumentToolCore(input) {
    ...
    switch (toolName) {
        case 'search_text': {
            ...
            if (!text)
                return fail('search_text 需要 text');
        ...
        case 'get_paragraph': {
            ...
            if (!paragraph)
                return fail(`未找到第 ${Number(params.index) + 1} 段`);
        ...
        case 'get_comments': {
            ...
            state.doc.forEach(node => {
                ...
                node.forEach(inline => {
                    ...
                    if (mark && inline.isText)
                        comments.push({ ...mark.attrs, paragraphIndex, markedText: inline.text ?? '' });
                });
                paragraphIndex += 1;
            });
        ...
        case 'set_text_style': {
            ...
            if (typeof attrs.fontFamily === 'string')
                attrs.fontFamily = FONT_MAP[attrs.fontFamily] ?? attrs.fontFamily;
        ...
        case 'set_paragraph_style': {
            ...
            if (paragraphs.length === 0)
                return fail('未找到可设置段落格式的范围');
            const attrs = Object.fromEntries(Object.entries(params).filter(([key, value]) => key !== 'range' && value !== undefined));
            if (attrs.listType === 'none')
                attrs.listType = null;
            if (attrs.headingLevel === 0 || attrs.headingLevel === 'none')
                attrs.headingLevel = null;
            for (const paragraph of paragraphs)
        ...
        case 'apply_style_batch': {
            ...
            if (rules.length === 0)
                return fail('apply_style_batch 需要至少一条规则');
            ...
            for (const rule of rules) {
                ...
                if (rule.textStyle && typeof rule.textStyle === 'object') {
                    ...
                    for (const match of resolveTextMatches(state.doc, range, selectionContext)) {
                        tr = addTextMark(tr, state, match.from, match.to, attrs);
                        if (match.paragraph.index >= 0)
                            affected.add(match.paragraph.index);
                    }
                }
        ...
        case 'set_page_config': {
            ...
            if (typeof params.paperSize === 'string') {
                ...
                if (size)
                    [nextPageConfig.pageWidth, nextPageConfig.pageHeight] = size;
            }
            if (params.orientation === 'landscape' && nextPageConfig.pageWidth < nextPageConfig.pageHeight)
                [nextPageConfig.pageWidth, nextPageConfig.pageHeight] = [nextPageConfig.pageHeight, nextPageConfig.pageWidth];
            if (params.orientation === 'portrait' && nextPageConfig.pageWidth > nextPageConfig.pageHeight)
                [nextPageConfig.pageWidth, nextPageConfig.pageHeight] = [nextPageConfig.pageHeight, nextPageConfig.pageWidth];
            for (const [key, param] of [['marginTop', 'marginTop'], ['marginBottom', 'marginBottom'], ['marginLeft', 'marginLeft'], ['marginRight', 'marginRight']]) {
                if (params[param] != null)
                    nextPageConfig[key] = Math.round(Number(params[param]) * 3.7795);
            }
        ...
        case 'begin_streaming_write': {
            ...
            if (!['insert_after_paragraph', 'replace_paragraph'].includes(action))
                return fail(`begin_streaming_write 的 action 无效：${action || '空值'}`);
            if (!markdown) {
            ...
            if (!paragraph)
                return fail(`未找到第 ${paragraphIndex + 1} 段`);
        ...
        case 'insert_text': {
            ...
            if (!paragraph)
                return fail(`未找到第 ${Number(params.paragraphIndex) + 1} 段`);
        ...
        case 'insert_paragraph_after': {
            ...
            if (!paragraph)
                return fail(`未找到第 ${Number(params.afterParagraph) + 1} 段`);
        ...
        case 'replace_selection_text': {
            ...
            if (!bounds)
                return fail('replace_selection_text 需要有效的 selection 范围');
        ...
        case 'delete_selection_text': {
            const bounds = resolveSelection(state.doc, params.range, selectionContext)[0];
            if (!bounds)
                return fail('delete_selection_text 需要有效的 selection 范围');
        ...
        case 'delete_paragraph': {
            ...
            if (indexes.length === 0)
                return fail('delete_paragraph 需要 index 或 indices');
            for (const index of [...indexes].sort((a, b) => b - a)) {
                const paragraph = paragraphAt(tr.doc, index);
                if (!paragraph)
                    return fail(`未找到第 ${index + 1} 段`);
                if (tr.doc.childCount === 1 && indexes.length === 1)
        ...
        case 'insert_image': {
            ...
            if (!src)
                return fail('insert_image 需要 src 参数');
        ...
        case 'insert_mermaid': {
            ...
            if (!code.trim())
                return fail('insert_mermaid 需要 code 参数');
            try {
