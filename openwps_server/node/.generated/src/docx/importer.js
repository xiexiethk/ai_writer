...
function resolveThemeFont(themeFonts, themeKey) {
    switch (themeKey) {
        case 'majorEastAsia':
...
function clampLineHeight(value, fallback = DEFAULT_PARAGRAPH_ATTRS.lineHeight) {
    if (!Number.isFinite(value))
        return fallback;
    return Math.min(3, Math.max(1, value));
}
function parseWordLineHeight(line, lineRule, baseFontSize, docGridLinePitchPt) {
    if (line <= 0)
        return DEFAULT_PARAGRAPH_ATTRS.lineHeight;
    if (lineRule === 'auto')
        return clampLineHeight(line / 240);
...
function twipToPt(value) {
    return value * TWIP_TO_PT;
}
function twipToEm(value, fontSizePt) {
    return twipToPt(value) / Math.max(fontSizePt, 1);
}
function hundredthCharToEm(value) {
    return value / 100;
}
function halfPtToPt(value) {
    return value / 2;
}
...
function truthyElement(element) {
    if (!element)
        return false;
...
function relTargetToZipPath(target) {
    return normalizePath(`word/${target}`);
}
function parseListType(raw) {
    if (!raw || raw === 'none')
        return null;
...
function createTextNode(text, attrs, extraMarks = []) {
    if (!text)
        return null;
    return {
        type: 'text',
...
function parseThemeFonts(themeXml) {
    ...
    const findScriptFont = (parent, script) => elementChildren(parent ?? dom).find((child) => getLocalName(child) === 'font' && getAttr(child, 'script') === script);
    return {
...
function readParagraphStyle(pPr, inherited, docGridLinePitchPt, numberingMap = {}) {
    if (!pPr)
        return {};
    const alignMap = {
        left: 'left',
    ...
    if (!shouldIgnoreWordIndents) {
        if (leftIndent > 0)
            attrs.indent = twipToEm(leftIndent, baseFontSize) / 2;
        else if (leftChars > 0)
            attrs.indent = hundredthCharToEm(leftChars) / 2;
        if (rightIndent > 0)
            attrs.rightIndent = twipToEm(rightIndent, baseFontSize) / 2;
        else if (rightChars > 0)
            attrs.rightIndent = hundredthCharToEm(rightChars) / 2;
        if (firstLine > 0)
            attrs.firstLineIndent = twipToEm(firstLine, baseFontSize);
        else if (firstLineChars > 0)
            attrs.firstLineIndent = hundredthCharToEm(firstLineChars);
        if (hanging > 0)
            attrs.firstLineIndent = -twipToEm(hanging, baseFontSize);
        else if (hangingChars > 0)
            attrs.firstLineIndent = -hundredthCharToEm(hangingChars);
    }
    ...
    if (before > 0)
        attrs.spaceBefore = twipToPt(before);
    if (after > 0)
        attrs.spaceAfter = twipToPt(after);
    if (line > 0) {
    ...
    if (truthyElement(directChild(pPr, 'pageBreakBefore')))
...
function readRunStyle(rPr, inherited, themeFonts) {
    if (!rPr)
        return { ...inherited };
    const attrs = {};
    const fonts = directChild(rPr, 'rFonts');
    const fontFamily = getAttr(fonts, 'eastAsia')
        || getAttr(fonts, 'ascii')
        || getAttr(fonts, 'hAnsi')
    ...
    if (fontFamily)
        attrs.fontFamily = normalizeFont(fontFamily);
    const size = parseNumber(getAttr(directChild(rPr, 'sz'), 'val'));
    if (size > 0)
        attrs.fontSize = halfPtToPt(size);
    const color = getAttr(directChild(rPr, 'color'), 'val');
    if (color)
        attrs.color = normalizeColor(color);
    const highlight = getAttr(directChild(rPr, 'highlight'), 'val');
    if (highlight)
        attrs.backgroundColor = highlightToColor(highlight) || attrs.backgroundColor || '';
    ...
    if (spacing !== 0)
        attrs.letterSpacing = twipToPt(spacing);
    if (truthyElement(directChild(rPr, 'b')))
        attrs.bold = true;
    if (truthyElement(directChild(rPr, 'i')))
        attrs.italic = true;
    if (truthyElement(directChild(rPr, 'u')))
        attrs.underline = true;
    if (truthyElement(directChild(rPr, 'strike')) || truthyElement(directChild(rPr, 'dstrike')))
...
function parseStyleDefaults(stylesXml, themeFonts) {
    if (!stylesXml)
        return {};
...
function parseStyles(stylesXml, themeFonts, docDefaults, numberingMap) {
    if (!stylesXml) {
        return {
            styleMap: {},
    ...
    for (const styleEl of Array.from(dom.getElementsByTagNameNS(W_NS, 'style'))) {
        ...
        if (headingLevel != null)
            paraAttrs.headingLevel = headingLevel;
    ...
    const resolveStyle = (styleId) => {
        ...
        if (resolving.has(styleId))
            return rawStyles[styleId]?.attrs ?? {};
...
function inferHeadingLevelFromStyle(styleId, styleName) {
    ...
    if (!raw)
        return null;
    const cjkMap = { 一: 1, 二: 2, 三: 3, 四: 4, 五: 5, 六: 6, 七: 7, 八: 8, 九: 9 };
...
function parseDocumentLayout(documentXml) {
...
function parseTypographySettings(settingsXml) {
    if (!settingsXml) {
        return {
...
async function parseImageNode(drawingEl, rels, zip) {
    ...
    if (isPict) {
        ...
        if (wMatch)
            widthPx = Number(wMatch[1]) * (wMatch[2] === 'px' ? 1 : 1.333333);
        const hMatch = style.match(/height:\s*([\d.]+)(pt|px)?/i);
        if (hMatch)
            heightPx = Number(hMatch[1]) * (hMatch[2] === 'px' ? 1 : 1.333333);
    }
    else {
...
function parseBooleanAttr(element, name, fallback = false) {
    const raw = getAttr(element, name);
    if (!raw)
        return fallback;
    return !['0', 'false', 'off', 'none'].includes(raw);
}
async function parseFloatingObjectNode(drawingEl, styleMap, defaultParagraphStyle, themeFonts, docGridLinePitchPt, numberingMap, rels, zip, commentMap = new Map()) {
    ...
    if (txbxContent) {
        ...
        for (let index = 0; index < paragraphElements.length; index += 1) {
            ...
            const parsed = await parseParagraph(paragraphEl, styleMap, defaultParagraphStyle, themeFonts, docGridLinePitchPt, numberingMap, rels, zip, commentMap);
            if (!parsed.node)
                continue;
            ...
            const isEmptyParagraph = parsed.node.type === 'paragraph' &&
...
async function parseRunChildNode(child, result, flushText, styleMap, paragraphStyle, themeFonts, docGridLinePitchPt, numberingMap, rels, zip, commentMap, linkHref, activeCommentMarks = []) {
    const localName = getLocalName(child);
    if (localName === 'drawing' || localName === 'pict') {
        flushText();
        const floatingObject = await parseFloatingObjectNode(child, styleMap, paragraphStyle, themeFonts, docGridLinePitchPt, numberingMap, rels, zip, commentMap);
        if (floatingObject) {
    ...
    if (localName === 'AlternateContent') {
        ...
        for (const variant of variants) {
            const nestedResult = { nodes: [], floatingObjects: [] };
            for (const nestedChild of elementChildren(variant)) {
                const nestedHandled = await parseRunChildNode(nestedChild, nestedResult, flushText, styleMap, paragraphStyle, themeFonts, docGridLinePitchPt, numberingMap, rels, zip, commentMap, linkHref, activeCommentMarks);
                if (nestedHandled)
                    continue;
                if (getLocalName(nestedChild) !== 'r')
                    continue;
                const nestedRun = await parseRunNodes(nestedChild, styleMap, paragraphStyle, themeFonts, docGridLinePitchPt, numberingMap, rels, zip, commentMap, linkHref, activeCommentMarks);
...
async function parseRunNodes(rEl, styleMap, paragraphStyle, themeFonts, docGridLinePitchPt, numberingMap, rels, zip, commentMap = new Map(), linkHref, activeCommentMarks = []) {
    const rPr = directChild(rEl, 'rPr');
    const styleId = getAttr(directChild(rPr, 'rStyle'), 'val');
    const effectiveStyle = readRunStyle(rPr, { ...paragraphStyle, ...(styleId ? styleMap[styleId] : {}) }, themeFonts);
    const extraMarks = [
        ...(linkHref ? [{ type: 'link', attrs: { href: linkHref } }] : []),
        ...activeCommentMarks,
    ];
    const result = { nodes: [], floatingObjects: [] };
    let textBuffer = '';
    const flushText = () => {
    ...
    for (const child of elementChildren(rEl)) {
        ...
        if (localName === 'sym') {
            ...
            if (Number.isFinite(parsed))
                textBuffer += String.fromCharCode(parsed);
            continue;
        }
        const handled = await parseRunChildNode(child, result, flushText, styleMap, paragraphStyle, themeFonts, docGridLinePitchPt, numberingMap, rels, zip, commentMap, linkHref, activeCommentMarks);
        if (handled)
            continue;
    }
    flushText();
...
async function parseParagraph(pEl, styleMap, defaultParagraphStyle, themeFonts, docGridLinePitchPt, numberingMap, rels, zip, commentMap = new Map()) {
    ...
    for (const child of elementChildren(pEl)) {
        ...
        const activeCommentMarks = Array.from(openCommentIds)
            .map((id) => {
        ...
        if (localName === 'r') {
            const parsedRun = await parseRunNodes(child, styleMap, paragraphStyle, themeFonts, docGridLinePitchPt, numberingMap, rels, zip, commentMap, undefined, activeCommentMarks);
        ...
        if (localName === 'hyperlink') {
            ...
            for (const run of directChildren(child, 'r')) {
                const parsedRun = await parseRunNodes(run, styleMap, paragraphStyle, themeFonts, docGridLinePitchPt, numberingMap, rels, zip, commentMap, linkHref || undefined, activeCommentMarks);
                content.push(...parsedRun.nodes);
                floatingObjects.push(...parsedRun.floatingObjects);
            }
        }
    ...
    const hasOnlyStructuralChildren = nonPropertyChildren.length > 0 &&
    ...
    const isStructuralOnlyParagraph = !hasVisibleInlineContent &&
...
async function parseTableCell(tcEl, isHeader, styleMap, defaultParagraphStyle, themeFonts, docGridLinePitchPt, numberingMap, rels, zip, commentMap = new Map()) {
    ...
    for (const child of elementChildren(tcEl)) {
        ...
        if (localName === 'p') {
            ...
            if (parsed.floatingObjects.length > 0)
                content.push(...parsed.floatingObjects);
        }
...
async function parseTable(tblEl, styleMap, defaultParagraphStyle, themeFonts, docGridLinePitchPt, numberingMap, rels, zip, commentMap = new Map()) {
    const rows = [];
    ...
    for (const trEl of directChildren(tblEl, 'tr')) {
        ...
        for (const tcEl of directChildren(trEl, 'tc')) {
            ...
            if (parsedCell.node) {
                ...
                for (let offset = 0; offset < colspan; offset += 1) {
                    if (hasRestart)
                        mergeAnchors.set(columnIndex + offset, parsedCell.node);
                    else
                        mergeAnchors.delete(columnIndex + offset);
                }
            }
            columnIndex += colspan;
        }
...
async function parseDocument(documentXml, styleMap, defaultParagraphStyle, themeFonts, docGridLinePitchPt, numberingMap, rels, zip, commentMap = new Map()) {
    const dom = parseXml(documentXml);
    ...
    for (const child of elementChildren(body)) {
        ...
        if (localName === 'p') {
            ...
            const isEmpty = !para.content || para.content.length === 0 ||
                para.content.every((n) => n.type === 'text' && !n.text?.trim());
            if (isEmpty && content.length === 0)
                continue;
            // 文档第一个有内容的段落去掉 spaceBefore（防止标题段前间距把内容往下推）
            const isFirstContent = content.length === 0 || content.every((n) => {
                if (n.type !== 'paragraph')
                    return false;
                return !n.content || n.content.length === 0 ||
                    n.content.every((c) => c.type === 'text' && !c.text?.trim());
            });
...
export async function importDocx(file) {
    ...
    const doc = await parseDocument(documentXml, styleMap, defaultParagraphStyle, themeFonts, docGridLinePitchPt, numberingMap, rels, zip, commentMap);
