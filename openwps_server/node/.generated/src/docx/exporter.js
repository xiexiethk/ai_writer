import { AlignmentType, BorderStyle, Document, DocumentGridType, ImageRun, ExternalHyperlink, HeadingLevel, LevelFormat, LineRuleType, Packer, PageOrientation, Paragraph, ShadingType, Table, TableCell, TableOfContents, TableRow, TextRun, UnderlineType, WidthType, } from 'docx';
import saveAs from 'file-saver';
import JSZip from 'jszip';
import { DEFAULT_PAGE_CONFIG, paginate } from '../layout/paginator.js';
import { toDocxFontName } from '../fonts.js';
const PX_TO_TWIP = 1440 / 96;
function pxToTwip(value) {
    return Math.round(value * PX_TO_TWIP);
}
function withXmlAttribute(tag, name, value) {
...
function paragraphAlignment(align) {
    const alignMap = {
...
async function convertInlineNode(node) {
...
async function convertParagraph(node, exportOptions) {
    const children = [];
    ...
    return new Paragraph({
        heading,
        alignment: paragraphAlignment(node.attrs.align),
...
async function convertTableCell(node, exportOptions) {
    const children = [];
    for (let index = 0; index < node.childCount; index += 1) {
...
async function convertTable(node, exportOptions) {
    const rows = [];
...
function collectTableOfContentsEntries(pmDoc, pageConfig, minLevel, maxLevel, pagination) {
...
function convertTableOfContents(node, pmDoc, pageConfig, pagination) {
...
async function convertNode(node, exportOptions, pmDoc, pageConfig, pagination) {
    if (node.type.name === 'table_of_contents')
        return convertTableOfContents(node, pmDoc, pageConfig, pagination);
    if (node.type.name === 'table')
        return convertTable(node, exportOptions);
    return convertParagraph(node, exportOptions);
}
function insertIntoSettingsXml(settingsXml, snippet, beforeTag = '<w:compat>') {
    if (settingsXml.includes(snippet))
        return settingsXml;
    if (settingsXml.includes(beforeTag))
        return settingsXml.replace(beforeTag, `${snippet}${beforeTag}`);
    return settingsXml.replace('</w:settings>', `${snippet}</w:settings>`);
}
/** Walk the PMNode tree and collect unique comments (deduped by id). */
function collectComments(pmDoc) {
    const seen = new Map();
 ...
 */
function patchDocumentXmlWithComments(documentXml, pmDoc) {
    const markedRuns = [];
    pmDoc.descendants((node) => {
    ...
    for (const { text, commentIds } of markedRuns) {
        ...
        const runPattern = new RegExp(`(<w:r(?:\\s[^>]*)?>(?:(?!<w:r(?:\\s|>)).)*?<w:t(?:[^>]*)?>)(${escapedText.replace(/[$()*+.[\]?\\^{}|]/g, '\\$&')})(</w:t>(?:(?!<w:r(?:\\s|>)).)*?</w:r>)`, 's');
...
function patchContentTypesForComments(contentTypesXml) {
...
async function patchDocxComments(blob, pmDoc) {
...
async function patchDocxSettings(blob, exportOptions) {
...
function patchDocumentXmlWithPageConfig(documentXml, pageConfig) {
...
async function patchDocxPageConfig(blob, pageConfig) {
    const zip = await JSZip.loadAsync(await blob.arrayBuffer());
    const documentFile = zip.file('word/document.xml');
    if (!documentFile)
        return blob;
...
export async function buildDocxBlob(pmDoc, pageConfig, exportOptions = {}, pagination) {
    const children = [];
    for (let index = 0; index < pmDoc.childCount; index += 1) {
        const child = pmDoc.child(index);
        if (child.type.name === 'floating_object')
            continue;
...
export async function exportDocx(blobOrDoc, pageConfig, exportOptions = {}, pagination) {
