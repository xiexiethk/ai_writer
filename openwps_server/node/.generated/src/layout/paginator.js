...
function resolveTextStyle(node, baseStyle = {}) {
...
function getParagraphTextStyle(paraNode) {
...
function getParagraphBaseTextStyle(paraNode) {
    const headingLevel = Number(paraNode.attrs.headingLevel ?? 0);
    const headingSizes = {
        1: 22,
        2: 18,
...
function textStyleToFontStr(style) {
...
function isLatinAlphaChar(char) {
    return LATIN_ALPHA_RE.test(char);
}
function shouldInsertMixedScriptGap(prevChar, nextChar) {
    return ((isLatinAlphaChar(prevChar) && isHanChar(nextChar)) ||
...
function measureAdvanceWidth(text, fontStr) {
...
function measureCompressedLineEndWidth(text, fontStr) {
    const ctx = getMeasureCanvasContext();
    if (!ctx)
        return measureAdvanceWidth(text, fontStr);
    ctx.font = fontStr;
    let totalWidth = 0;
    for (const char of Array.from(text)) {
        const metrics = ctx.measureText(char);
        const inkWidth = Math.max(0, (metrics.actualBoundingBoxLeft ?? 0) + (metrics.actualBoundingBoxRight ?? 0));
        totalWidth += inkWidth > 0 ? Math.min(metrics.width, inkWidth) : metrics.width;
    }
    return totalWidth;
}
function normalizeMeasuredChar(char) {
    return char;
}
function isCompressiblePunctuation(char) {
    return COMPRESSIBLE_PUNCT_RE.test(char);
}
function isOpeningPunctuation(char) {
    return OPENING_PUNCT_RE.test(char);
}
function isEndAnchoredPunctuation(char) {
    return END_ANCHORED_PUNCT_RE.test(char);
}
function getPunctuationAnchor(char) {
    if (isOpeningPunctuation(char))
        return 'end';
    if (isEndAnchoredPunctuation(char))
        return 'start';
    return 'start';
}
function extractParagraphSourceChars(paraNode, paraPos) {
    if (!paragraphHasOnlyTextInlines(paraNode))
        return null;
    const chars = [];
...
function splitSourceCharsByNewline(chars) {
    const groups = [[]];
    chars.forEach((char) => {
        if (char.char === '\n') {
            groups.push([]);
...
function parseParagraphTabStops(rawTabStops) {
    if (!Array.isArray(rawTabStops))
        return [];
    return rawTabStops
...
function assignMeasuredWidths(units) {
    if (!units.length)
        return;
    let startIndex = 0;
    while (startIndex < units.length) {
        ...
        try {
            ...
            const widths = prepared.segments.flatMap((segment, index) => (segment === '\u200b' ? [] : [prepared.widths[index] ?? 0]));
            if (widths.length === group.length)
...
function buildMeasureUnits(chars) {
    if (!chars.length)
        return [];
    const units = [];
...
function buildTabbedLine(chars, tabStops, availableWidth) {
    const segments = [[]];
    chars.forEach((char) => {
        if (char.char === '\t') {
            segments.push([]);
...
function fitUnitsToLines(units, layoutWidth, firstLineWidth = layoutWidth) {
    if (units.length === 0) {
        return [{
                text: '',
                sourceCharCount: 0,
    ...
    const canFitWithCompression = (extraUnits) => {
    ...
    const collectTailPunctuationUnits = (startIndex) => {
        ...
        for (let index = startIndex; index < units.length; index += 1) {
            ...
            if (!canFitWithCompression(nextCollected))
                break;
            collected.push(unit);
        }
        return collected;
    };
    const shouldMoveOpeningPunctuationToNextLine = (index) => {
        const unit = units[index];
        if (!unit || !isOpeningPunctuation(unit.displayText) || currentUnits.length === 0)
            return false;
    ...
    const buildRenderedUnits = (lineUnits, overflow) => {
        if (overflow <= 0) {
...
function estimateImageHeight(node) {
...
function measureParagraph(paraNode, paraPos, blockIndex, contentWidth, orderedListIndex = 0) {
    const { fontSize } = getParagraphTextStyle(paraNode);
    const align = (paraNode.attrs.align ?? 'left');
    const lineHeightMult = paraNode.attrs.lineHeight ?? 1.5;
    const spaceBefore = ptToPx(paraNode.attrs.spaceBefore ?? 0);
    const spaceAfter = ptToPx(paraNode.attrs.spaceAfter ?? 0);
    const paragraphIndentPx = Math.max(0, ptToPx(fontSize * ((paraNode.attrs.indent ?? 0) * 2)));
    const paragraphRightIndentPx = Math.max(0, ptToPx(fontSize * ((paraNode.attrs.rightIndent ?? 0) * 2)));
    const firstLineIndentPx = Math.max(0, ptToPx(fontSize * (paraNode.attrs.firstLineIndent ?? 0)));
    const rawListType = paraNode.attrs.listType ?? null;
    const listType = rawListType === 'task' || rawListType === 'bullet' || rawListType === 'ordered' ? rawListType : null;
    ...
    try {
        ...
        fittedGroups = (groups.length ? groups : [[]]).map((groupChars, groupIndex) => {
            if (groupChars.some((char) => char.char === '\t') && tabStops.length > 0) {
                return {
                    lines: [buildTabbedLine(groupChars, tabStops, groupIndex === 0 ? firstLineWidth : layoutWidth)],
                };
            }
...
function resolveFloatingLeft(node, config, contentWidth) {
...
function resolveFloatingTop(node, anchorTop, config) {
...
function buildFloatingParagraphs(node) {
    const paragraphs = Array.isArray(node.attrs.paragraphs) ? node.attrs.paragraphs : [];
    return paragraphs
        .filter((paragraph) => Boolean(paragraph) && typeof paragraph === 'object')
        .map((paragraph) => {
        ...
        const runs = rawContent
            ...
            const textStyleMark = marks.find((mark) => (Boolean(mark) &&
                typeof mark === 'object' &&
                mark.type === 'textStyle'));
            const commentMark = marks.find((mark) => (Boolean(mark) &&
                typeof mark === 'object' &&
                mark.type === 'comment'));
            const linkMark = marks.find((mark) => (Boolean(mark) &&
                typeof mark === 'object' &&
                mark.type === 'link'));
...
function buildRenderedFloatingObject(node, blockIndex, blockPos, anchorTop, config, contentWidth) {
...
function getFloatingFlowFloor(page, config) {
    return page.floatingObjects.reduce((maxBottom, object) => {
        if (object.behindDoc || object.wrap !== 'none')
            return maxBottom;
...
function getDomBlockMetric(metrics, nodePos, blockIndex, blockType) {
    return metrics.find((metric) => (metric.blockType === blockType
        && (metric.pos === nodePos || metric.blockIndex === blockIndex)));
}
function measureTableCell(cellNode, cellContentWidth) {
    let totalHeight = 0;
...
function countRowColumns(rowNode) {
    let count = 0;
...
function buildTablePreviewText(tableNode) {
    const text = tableNode.textContent.replace(/\s+/g, ' ').trim();
    if (!text)
        return '[表格]';
    return text.length > 180 ? `${text.slice(0, 180)}...` : text;
}
function getFirstParagraphTextPos(cellNode, cellPos) {
    let found = null;
...
function buildRenderedTable(tableNode, tablePos, contentWidth, contentHeight, domMetric) {
...
function measureTable(tableNode, tablePos, contentWidth, domMetric) {
    if (domMetric) {
    ...
    tableNode.forEach((rowNode) => {
        ...
        rowNode.forEach((cellNode) => {
            rowHeight = Math.max(rowHeight, measureTableCell(cellNode, cellContentWidth) + TABLE_CELL_VERTICAL_CHROME_PX);
        });
...
function normalizeHeadingLevel(rawLevel) {
...
function collectDocumentHeadings(doc) {
    ...
    doc.forEach((node) => {
        if (node.type.name === 'paragraph') {
            ...
            if (level != null && title)
                headings.push({ title, level, blockIndex });
        }
        blockIndex += 1;
    });
    return headings;
}
function getTableOfContentsLevelRange(node) {
...
function getTableOfContentsEntries(node, headings) {
...
function measureBlock(node, nodePos, blockIndex, contentWidth, headings = [], domBlockMetrics = [], orderedListIndex = 0) {
    switch (node.type.name) {
        case 'paragraph':
            return measureParagraph(node, nodePos, blockIndex, contentWidth, orderedListIndex);
        case 'table': {
            const measured = measureTable(node, nodePos, contentWidth, getDomBlockMetric(domBlockMetrics, nodePos, blockIndex, 'table'));
...
function pushPageBreak(breaks, pages, currentPage, breakPos) {
    breaks.push({
        pos: breakPos,
        pageIndex: pages.length,
        prevPageUsed: currentPage.totalHeight,
    });
    const nextPage = { lines: [], totalHeight: 0 };
    pages.push(nextPage);
    return nextPage;
}
function pushRenderedLines(renderedPage, measured, startLineIndex, endLineIndex, lastLineIndex) {
...
function lineNeededHeight(measured, lineIndex, lastLineIndex) {
    const line = measured.lines[lineIndex];
    const extraBefore = lineIndex === 0 ? measured.spaceBefore : 0;
    const extraAfter = lineIndex === lastLineIndex ? measured.spaceAfter : 0;
    return extraBefore + line.lineHeight + extraAfter;
}
export function paginate(doc, config = DEFAULT_PAGE_CONFIG, options = {}) {
    ...
    const pages = [{ lines: [], totalHeight: 0 }];
    const renderedPages = [{ lines: [], totalHeight: 0, floatingObjects: [] }];
    const breaks = [];
    const lineBreakSet = new Set();
    ...
    doc.forEach((node, offset) => {
        ...
        if (node.type.name === 'floating_object') {
            currentRenderedPage.floatingObjects.push(buildRenderedFloatingObject(node, blockIndex, nodePos, lastAnchorTop, config, contentWidth));
            blockIndex += 1;
            return;
        }
        ...
        const measured = measureBlock(node, nodePos, blockIndex, contentWidth, documentHeadings, domBlockMetrics, orderedListIndex);
        ...
        while (startLineIndex < measured.lines.length) {
            ...
            for (let lineIndex = startLineIndex; lineIndex < measured.lines.length; lineIndex += 1) {
                ...
                if (currentPage.totalHeight + heightSum + neededHeight > contentHeight)
                    break;
                fitCount += 1;
            ...
            if (fitCount === 0) {
                if (currentPage.lines.length > 0) {
                    currentPage = pushPageBreak(breaks, pages, currentPage, measured.lines[startLineIndex].startPos ?? nodePos);
                    currentRenderedPage = { lines: [], totalHeight: 0, floatingObjects: [] };
                    renderedPages.push(currentRenderedPage);
                    lastAnchorTop = 0;
                    continue;
            ...
            pushRenderedLines(currentRenderedPage, measured, startLineIndex, startLineIndex + fitCount, lastLineIndex);
    ...
    console.log(`[paginator] ${pages.length} page(s), contentHeight=${contentHeight}px, ` +
    ...
    renderedPages.forEach((page, pageIndex) => {
        page.lines.forEach((line) => {
            if (!blockPageMap.has(line.blockIndex))
                blockPageMap.set(line.blockIndex, pageIndex + 1);
        });
    });
